...
def _safe_savgol(series, window=7, polyorder=1):
    """
    Aplica Savitzky-Golay de forma robusta em séries com NaN.
    """
    values = series.to_numpy(dtype=float)
    if values.size == 0 or np.isnan(values).all():
        return series

    # Interpola lacunas para evitar erro do scipy com NaN/inf.
    valid = ~np.isnan(values)
    x = np.arange(values.size)
    values_filled = values.copy()
    values_filled[~valid] = np.interp(x[~valid], x[valid], values[valid])

    # Ajusta janela para o tamanho do trecho e garante ímpar.
    win = min(window, values_filled.size)
    if win % 2 == 0:
        win -= 1
    if win <= polyorder:
        win = polyorder + 2 + (polyorder % 2)
    if win > values_filled.size:
        win = values_filled.size if values_filled.size % 2 == 1 else values_filled.size - 1
    if win <= polyorder or win < 3:
        return series

    filtered = signal.savgol_filter(values_filled, window_length=win, polyorder=polyorder)
    return series.__class__(filtered, index=series.index)
    ...
    for player in player_ids: # cycle through players individually
        ...
        if smoothing:
            if filter_=='Savitzky-Golay':
                ...
                vx.loc[:second_half_idx] = _safe_savgol(vx.loc[:second_half_idx], window=window, polyorder=polyorder)
                vy.loc[:second_half_idx] = _safe_savgol(vy.loc[:second_half_idx], window=window, polyorder=polyorder)
